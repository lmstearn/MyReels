...
DefaultDirName={#MyDefaultDir}{#MyAppName}
DefaultGroupName={#MyAppName}
; Note: The Installer script resides in the Installer directory!
OutputDir={#MySetupDir}{#MyAppName}\Installer
...
Source: "..\*"; DestDir: "{app}"; Flags: touch; Excludes: "Readme.txt,slotdata.s$t,HallFame.s$t,{#MyAppExeName},{#MyAppSysExeName},{#MyAppUpdaterExeName},{#MyAppName}.chm,*.mid"
...
Source: "{#MyAppUpdaterExeName}"; DestDir: "{app}"; Flags: touch promptifolder; Components: program
Source: "Configuration\slotdata.s$t"; DestDir: "{app}"; Flags: touch; Components: program\configuration
Source: "Configuration\HallFame.s$t"; DestDir: "{app}"; Flags: touch; Components: program\configuration
...
[Run]
Filename: "{tmp}\{#MyAppSysExeName}"; Parameters: "/silent /{#MyAppName}Dir={app}"; Flags: waituntilterminated shellexec; Description: "Install {#MyAppName} sys files"; StatusMsg: "Installing {#MyAppName} sys files ..."; Verb: "runas"
...
// 3 - successfully executed the UnInstallString
  ...
  sUnInstallString := GetUninstallString();
    ...
    MsgBox('An ancient version of ' + ExpandConstant('{#MyAppName}') + ' detected! Click OK to commence uninstall but note:' +NL+ 'You will probably get a prompt to run as admin, unless already elevated.' +NL+ 'If choosing custom uninstall do not choose rollback system files!' +NL+ 'If there is a restart computer prompt, do not restart now! It is only for the UNWISE.EXE, old ' + ExpandConstant('{#MyAppName}') + ' FTS and GID files which may be already deleted.' +NL+ 'Do not cancel the uninstall particularly if the directory you have just selected was used as the ' + ExpandConstant('{#MyAppName}') + 'ls base directory!', mbInformation, MB_OK);
    ...
    if ShellExec('open', sUnInstallString, sUnInstallStringPrm,'', SW_SHOW, ewWaitUntilTerminated, iResultCode) then
     ...
     if MsgBox('Deleting the old ' + ExpandConstant('{#MyAppName}') + ' backup folder is recommended:', mbConfirmation, MB_YESNO) = IDYES Then
